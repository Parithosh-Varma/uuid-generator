        ...
        <div className="border border-border rounded-2xl bg-card shadow-sm overflow-hidden mb-6">
            ...
            <div className="flex items-center gap-2">
              {(['txt', 'json', 'csv'] as const).map((f) => (
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-bold uppercase tracking-wide rounded-lg border border-border bg-background hover:bg-muted text-foreground transition-all disabled:opacity-40"
                >
                  .{f}
                </button>
              ))}
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-muted/30">
            ...
            <button
              onClick={() => copy('all')}
              disabled={uuids.length === 0}
              className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity disabled:opacity-40"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-3.5 h-3.5"><rect width="14" height="14" x="8" y="8" rx="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
              Copy all
            </button>
